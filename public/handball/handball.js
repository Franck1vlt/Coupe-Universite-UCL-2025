...
function connectToServer() {
    try {
        ...
        socket.on('connect', () => {
            ...
            // Rejoindre la room du sport handball
            socket.emit('join_sport', 'handball');
...
async function resetGame() {
    ...
    try {
        ...
        // Mettre à jour directement le tournamentState dans localStorage
        const tournamentState = JSON.parse(localStorage.getItem('handballTournamentState') || '{}');
        ...
        window.location.href = 'handball.html';
        ...
        if (matchData.matchId) {
            ...
            if (action === 'match_completed' || action === 'update_match_status') {
                ...
                if (tournamentState.matches && tournamentState.matches[matchData.matchId]) {
                    ...
                    localStorage.setItem('handballTournamentState', JSON.stringify(tournamentState));
                    console.log("État du tournoi mis à jour dans localStorage:", tournamentState.matches[matchData.matchId]);
                }
            }
        }
    } catch (error) {
...
// Ajouter cette fonction avant reset game dans handball.js
function debugTournamentState() {
    const tournamentState = JSON.parse(localStorage.getItem('handballTournamentState') || '{}');
    console.log("État actuel du tournoi:", tournamentState);
    if (tournamentState.matches && matchData.matchId) {
        console.log("Match actuel:", tournamentState.matches[matchData.matchId]);
    }
}
